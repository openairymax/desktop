...
import {
  Wrench, Plus, Search, X, Loader2, RefreshCw, AlertCircle,
  Play, Trash2, Eye, ChevronDown
} from 'lucide-react';
...
const SkillRegistry: React.FC = () => {
  const { skills, loading, error: skillsError, fetchSkills, registerSkill, loadSkill, executeSkill, unloadSkill, deleteSkill, getSkillCount } = useSkills();
  ...
  const handleLoad = async (skillId: string) => {
    ...
    try { await loadSkill(skillId); } finally { setActionLoading(null); }
  };

  const handleUnload = async (skillId: string) => {
    setActionLoading(`unload-${skillId}`);
    try { await unloadSkill(skillId); } finally { setActionLoading(null); }
  };

  const handleDelete = async (skillId: string) => {
    setActionLoading(`delete-${skillId}`);
    try { await deleteSkill(skillId); } finally { setActionLoading(null); }
  };
    ...
    <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px', flexWrap: 'wrap', gap: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <p style={{ margin: '2px 0 0 0', color: 'var(--text-muted)' }}>AgentOS 技能管理、注册与执行</p>
          </div>
        </div>
      ...
      {skillsError && (
        <div style={{
          ...
          <button style={{ marginLeft: 'auto', background: 'none', border: 'none', color: 'inherit', cursor: 'pointer' }} onClick={() => fetchSkills()}>重试</button>
        </div>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        {[
          { label: '技能总数', value: stats.total, icon: <Wrench size={18} />, color: '#f59e0b', bg: 'rgba(245,158,11,0.1)' },
          { label: '已加载', value: stats.active, icon: <Play size={18} />, color: 'var(--success-color)', bg: 'var(--success-light)' },
          { label: '未激活', value: stats.inactive, icon: <ChevronDown size={18} />, color: 'var(--text-muted)', bg: 'var(--bg-tertiary)' },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '240px', position: 'relative' }}>
          ...
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="搜索技能..." style={{
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <div style={{
                        ...
                        background: 'linear-gradient(135deg, #f59e0b, #f97316)', display: 'flex',
                      ...
                      <div>
                        ...
                        <span style={{
                          ...
                          <span style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: statusCfg.color, display: 'inline-block' }} />
                          {statusCfg.label}
                        </span>
                      </div>
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <button onClick={() => openExecute(skill)} style={{
                    ...
                    {(skill.status === 'active' || skill.status === 'loaded') ? (
                      <button onClick={() => handleUnload(skill.id)} disabled={actionLoading === `unload-${skill.id}`} style={{
                        flex: 1, padding: '8px 12px', border: '1px solid var(--warning-color)', borderRadius: 'var(--radius-md)',
                        backgroundColor: 'var(--warning-light)', color: 'var(--warning-color)', cursor: 'pointer',
                        fontFamily: 'inherit', fontSize: 'var(--font-size-sm)', display: 'flex', alignItems: 'center',
                        justifyContent: 'center', gap: '4px', transition: 'all var(--transition-fast)',
                        opacity: actionLoading === `unload-${skill.id}` ? 0.5 : 1,
                      }}>
                        {actionLoading === `unload-${skill.id}` ? <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> : <ChevronDown size={14} />}
                        卸载
                      </button>
                    ) : (
                      <button onClick={() => handleLoad(skill.id)} disabled={actionLoading === `load-${skill.id}`} style={{
                        flex: 1, padding: '8px 12px', border: '1px solid var(--success-color)', borderRadius: 'var(--radius-md)',
                        backgroundColor: 'var(--success-light)', color: 'var(--success-color)', cursor: 'pointer',
                        fontFamily: 'inherit', fontSize: 'var(--font-size-sm)', display: 'flex', alignItems: 'center',
                        justifyContent: 'center', gap: '4px', transition: 'all var(--transition-fast)',
                        opacity: actionLoading === `load-${skill.id}` ? 0.5 : 1,
                      }}>
                        {actionLoading === `load-${skill.id}` ? <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> : <Play size={14} />}
                        加载
                      </button>
                    )}
                    <button onClick={() => handleDelete(skill.id)} disabled={actionLoading === `delete-${skill.id}`} style={{
                      padding: '8px 10px', border: '1px solid var(--error-color)', borderRadius: 'var(--radius-md)',
                      backgroundColor: 'var(--error-light)', color: 'var(--error-color)', cursor: 'pointer',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      transition: 'all var(--transition-fast)', opacity: actionLoading === `delete-${skill.id}` ? 0.5 : 1,
                    }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <h2 style={{ margin: 0, fontSize: 'var(--font-size-xl)', fontWeight: 'var(--font-weight-bold)', color: 'var(--text-primary)' }}>注册技能</h2>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
                <div>
                  ...
                  <input type="text" value={newName} onChange={e => setNewName(e.target.value)} placeholder="输入技能名称..." style={{
                ...
                <div>
                  ...
                  <textarea value={newDescription} onChange={e => setNewDescription(e.target.value)} rows={3} placeholder="描述技能的功能..." style={{
                    width: '100%', padding: '10px 14px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)',
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                ...
                <button onClick={handleRegister} disabled={!newName.trim() || actionLoading === 'register'} style={{
                  padding: '8px 16px', background: 'linear-gradient(135deg, #f59e0b, #f97316)', color: 'white', border: 'none',
                  borderRadius: 'var(--radius-md)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--font-size-md)',
                  display: 'flex', alignItems: 'center', gap: '6px', opacity: (!newName.trim() || actionLoading === 'register') ? 0.5 : 1,
                }}>
                  {actionLoading === 'register' ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Plus size={16} />}
                  注册
                </button>
              </div>
            </motion.div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <div>
                  <h2 style={{ margin: 0, fontSize: 'var(--font-size-xl)', fontWeight: 'var(--font-weight-bold)', color: 'var(--text-primary)' }}>执行技能</h2>
                  <p style={{ margin: '2px 0 0 0', fontSize: 'var(--font-size-sm)', color: 'var(--text-muted)' }}>{selectedSkill.name}</p>
                </div>
                <button onClick={() => setShowExecuteModal(false)} style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                  <X size={18} />
                </button>
              </div>
              <div>
                ...
                <textarea value={execParams} onChange={e => setExecParams(e.target.value)} rows={6} placeholder='{"key": "value"}' style={{
                  width: '100%', padding: '12px 14px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)',
                  backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-primary)', fontSize: 'var(--font-size-sm)', fontFamily: 'var(--font-mono)', outline: 'none', resize: 'vertical', marginBottom: '16px', transition: 'all var(--transition-fast)',
                }}
                onFocus={(e) => { e.currentTarget.style.borderColor = '#f59e0b'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(245,158,11,0.2)'; }}
                onBlur={(e) => { e.currentTarget.style.borderColor = 'var(--border-color)'; e.currentTarget.style.boxShadow = 'none'; }}
                />
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                <button onClick={() => setShowExecuteModal(false)} style={{ padding: '8px 16px', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-secondary)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--font-size-md)' }}>取消</button>
                <button onClick={handleExecute} disabled={actionLoading === 'execute'} style={{
                  padding: '8px 16px', background: 'linear-gradient(135deg, #f59e0b, #f97316)', color: 'white', border: 'none',
                  borderRadius: 'var(--radius-md)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--font-size-md)',
                  display: 'flex', alignItems: 'center', gap: '6px', opacity: actionLoading === 'execute' ? 0.5 : 1,
                }}>
                  {actionLoading === 'execute' ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Play size={16} />}
                  执行
                </button>
              </div>
            </motion.div>

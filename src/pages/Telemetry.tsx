...
const MiniChart: React.FC<{ data: MetricPoint[]; color: string; height?: number }> = ({
  data,
  color,
  height = 40,
}) => {
      ...
      setMetrics((prev) =>
        prev.map((m) => {
          ...
          const newValue = Math.max(
            m.min,
            Math.min(m.max, m.current + (Math.random() - 0.5) * variance * 0.2),
          );
          const newHistory = [...m.history.slice(1), { timestamp: Date.now(), value: newValue }];
          const newStatus =
            newValue > m.threshold * 0.9
              ? 'warning'
              : newValue > m.threshold
                ? 'critical'
                : 'normal';
          return {
            ...m,
              ...
              >
                ...
                >
                  ...
                  >
                    <TrendingUp
                      size={12}
                      style={{ transform: metric.change < 0 ? 'rotate(180deg)' : 'none' }}
                    />
              ...
              >
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                        状态:{' '}
                        <span style={{ color: getStatusColor(metric.status), fontWeight: '500' }}>
                          {metric.status}
                        </span>
                      </div>
                    </div>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div
                      style={{ fontSize: '36px', fontWeight: '700', color: 'var(--text-primary)' }}
                    >
                      {metric.current}
                      {metric.unit}
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: '12px', flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <span
                          style={{ textTransform: 'capitalize', color: getLevelColor(alert.level) }}
                        >
                          级别:{' '}
                          {alert.level === 'info'
                            ? '信息'

...
function toDisplaySession(s: Session, t: (key: string, opts?: Record<string, unknown>) => string): DisplaySession {
  return {
    id: s.id,
    name:
      (s.context?.name as string) ||
      (s.metadata?.name as string) ||
      t('sessionExtended.defaultSessionName', { id: s.id.slice(0, 8) }),
    agent:
      (s.context?.agent as string) ||
      (s.metadata?.agent as string) ||
      s.userId ||
      t('sessionExtended.defaultAgent'),
    status:
...
const DEFAULT_AGENTS = [{ id: 'auto', name: 'Auto Select' }];
    ...
    <div role="region" aria-label="会话管理">
      ...
      >
        <div className="grid-4" style={{ flex: 1, maxWidth: '600px' }}>
          ...
          <StatCard
            label={t('sessionExtended.inProgress')}
            value={stats.active}
            color="var(--success)"
          />
          <StatCard
            label={t('sessionExtended.completedSessions')}
            value={stats.completed}
            color="var(--info)"
          />
          <StatCard
            label={t('sessionExtended.archivedSessions')}
            ...
            {filtered.map((session, i) => (
              ...
              >
                ...
                >
                  <MessageCircle
                    size={18}
                    style={{ color: session.status === 'active' ? 'white' : 'var(--text-muted)' }}
                  />
                </div>
          ...
          <div className="modal-overlay" onClick={() => setSelectedSession(null)}>
            ...
            >
              ...
              >
                ...
                <DetailRow
                  label="Token 用量"
                  value={`${selectedSession.tokens.toLocaleString()} (${(selectedSession.tokens / 1000).toFixed(1)}K)`}
                />
...
function StatCard({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: '24px', fontWeight: 800, color: color || 'var(--text-primary)' }}>
        {value}
      </div>
    </div>
  );

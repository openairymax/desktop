...
import {
  Shield, AlertTriangle, CheckCircle, XCircle, Lock, FileText, Search,
  Filter, Bell, Settings, Eye, RefreshCw, ChevronDown, ChevronRight,
  User, Clock, Activity, Key, Database
} from 'lucide-react'
...
const defaultLogs: AuditLog[] = [
  { id: '1', timestamp: '2026-04-26 14:23:45', user: 'admin', action: '修改权限策略', resource: 'permission-policy', result: 'success', details: '更新了 agent-executor 的读写权限' },
  { id: '2', timestamp: '2026-04-26 13:45:12', user: 'agent-devops', action: '访问受限资源', resource: 'system-config', result: 'denied', details: '尝试访问系统配置被拒绝' },
  { id: '3', timestamp: '2026-04-26 12:30:00', user: 'system', action: '自动数据清洗', resource: 'input-sanitizer', result: 'success', details: '成功清洗 156 条输入数据' },
  { id: '4', timestamp: '2026-04-26 11:15:33', user: 'agent-researcher', action: '执行工具调用', resource: 'web-search', result: 'error', details: '工具调用超时，已自动重试' },
  { id: '5', timestamp: '2026-04-26 10:02:18', user: 'admin', action: '更新安全策略', resource: 'access-control', result: 'success', details: '新增 3 条访问控制规则' }
]
...
const SecurityCenter: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'enabled': return 'var(--success-color)'
  ...
  const getResultColor = (result: string) => {
    switch (result) {
      ...
      case 'denied': return 'var(--warning-color)'
  ...
  const getResultIcon = (result: string) => {
    switch (result) {
      ...
      case 'denied': return <AlertTriangle size={16} />
    ...
    <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '700', margin: '0 0 4px 0', color: 'var(--text-primary)' }}>
          <Shield size={24} style={{ marginRight: '8px', verticalAlign: 'middle', color: 'var(--primary-color)' }} />
          安全中心
        </h1>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '8px' }}>
        <button
          ...
          style={{
            ...
            cursor: 'pointer', fontSize: '13px', fontWeight: activeTab === 'policies' ? '500' : '400',
        ...
        <button
          ...
          style={{
            ...
            cursor: 'pointer', fontSize: '13px', fontWeight: activeTab === 'logs' ? '500' : '400',
            transition: 'all 150ms ease'
          }}
        >
          <FileText size={16} />
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    <div style={{
                      ...
                      justifyContent: 'center', color: getStatusColor(policy.status)
                    }}>
                      {policy.icon}
                    </div>
                    <div>
                      <h3 style={{ fontSize: '15px', fontWeight: '600', margin: '0 0 4px 0', color: 'var(--text-primary)' }}>
                        {policy.name}
                      </h3>
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                        状态: <span style={{ color: getStatusColor(policy.status), fontWeight: '500' }}>{policy.status === 'enabled' ? '已启用' : policy.status === 'disabled' ? '已禁用' : '警告'}</span>
                      </div>
                    </div>
                  </div>
                  <button
                    ...
                    style={{
                      ...
                      background: policy.status === 'enabled' ? 'var(--success-light)' : 'var(--bg-tertiary)',
                      color: policy.status === 'enabled' ? 'var(--success-color)' : 'var(--text-secondary)',
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>已禁用</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '24px', fontWeight: '700', color: 'var(--warning-color)' }}>
                  {policies.filter(p => p.status === 'warning').length}
                </div>
                <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>警告</div>
              </div>
              <div style={{ textAlign: 'center' }}>
              ...
              {['all', 'success', 'denied', 'error'].map(result => (
                <button
                  ...
                  style={{
                    ...
                    color: filterResult === result ? 'var(--primary-color)' : 'var(--text-secondary)',
                    cursor: 'pointer', fontSize: '12px', transition: 'all 150ms ease'
                  }}
                >
                  {result === 'all' ? '全部' : result === 'success' ? '成功' : result === 'denied' ? '拒绝' : '错误'}
                </button>
              ))}
            </div>
          </div>
                  ...
                  <div style={{ display: 'flex', gap: '12px', flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: 'var(--text-muted)', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ color: getResultColor(log.result), textTransform: 'capitalize', fontWeight: '500' }}>
                          {log.result === 'success' ? '成功' : log.result === 'denied' ? '拒绝' : '错误'}
                        </span>
                      </div>
                    </div>
                  </div>

        ...
        if (step.toolCall) {
          ...
          try {
            ...
            setTools((prev) =>
              prev.map((t) =>
                t.id === toolId
                  ? { ...t, status: 'error' as const, output: 'Tool execution failed' }
                  : t,
              ),
            );
          }
          ...
          <div
            ...
            style={{
              ...
              background: isRunning
                ? PHASES[currentPhase]?.gradient || 'var(--bg-tertiary)'
                : 'var(--bg-tertiary)',
              display: 'flex',
              ...
              boxShadow: `0 8px 32px ${PHASES[currentPhase]?.color || '#94a3b8'}25`,
              zIndex: 2,
            }}
          >
            {isRunning ? (
          ...
          >
            {thoughts.length === 0 ? (
              ...
              thoughts.map((thought, idx) => {
                const phaseData =
                  (PHASES as Record<string, StepData>)[thought.phase] || PHASES.idle;
                const IconComp = phaseData.icon;
                const isActive = activeThoughtIdx === idx && isRunning;
                return (
                  <div
                    key={`${idx}-${thought.timestamp}`}
                    ...
                    style={{
                      ...
                      borderRadius: 'var(--radius-md)',
                      cursor: 'pointer',
                      background: isActive ? `${phaseData.color}15` : 'transparent',
                      borderLeft: `3px solid ${isActive ? phaseData.color : phaseData.color}40`,
                      transition: 'all var(--transition-fast)',
                      animation: !isRunning
                        ? `staggerFadeIn 0.3s ease-out ${idx * 60}ms both`
                        : undefined,
                    }}
                  >
                    <div
                      style={{
                        ...
                        background: isActive ? phaseData.gradient : `${phaseData.color}15`,
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        flexShrink: 0,

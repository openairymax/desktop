...
> 桌面应用，将 AgentRT 后端核心模块能力映射到原生图形界面。
...
（kernel、daemons、gateway、OpenLab）映射为分页式图形工作区。
...
├── package.json               # npm 清单（名称 `airymax-agentos`）
...
桌面客户端将每个工作区映射到对应的 AgentRT 后端能力。下表列出各工作区标签页、
其背后的 AgentRT 组件及简要说明。

| 模块 | 对应 AgentRT 后端 | 功能描述 |
|------|-------------------|----------|
| 仪表盘 | 网关状态 API | 系统概览、实时状态、快速操作入口 |
| 智能体管理 | `daemons/agent_d` + `atoms/corekern` | 创建 / 启动 / 监控 / 销毁 AI 智能体 |
| 任务管理 | `atoms/taskflow` | 任务提交、调度、跟踪与结果查看 |
| 会话管理 | `daemons/agent_d` | 内置智能体类型的全生命周期管理 |
| AI 对话 | `daemons/llm_d` + `daemons/think_d` | 多智能体对话、上下文感知、建议提示 |
| 技能注册 | 内置技能 / hook 服务 | 技能注册与状态管理 |
| 工具管理 | `daemons/tool_d` + `atoms/syscall` | 工具注册、执行和权限控制 |
| 模型配置 | `daemons/llm_d` | LLM 提供商、系统参数、环境变量 |
| 认知循环 | `atoms/coreloopthree` | 4 阶段认知可视化、推理引擎选择 |
| 记忆系统 | `atoms/memory/`（`builtin` + `memoryrovol` 提供者） | L1-L4 多层记忆：检索、演化、清理 |
| 应用市场 | `daemons/market_d` | 扩展应用的搜索 / 安装 / 评分 |
| 服务网关 | `gateway/` + `daemons/gateway_d` | 后端服务健康与延迟监控 |
| 安全中心 | 运行时策略与审计服务 | 安全策略与审计日志 |
| 系统监控 | `daemons/monit_d` | 实时性能与环境指标 |
| 遥测仪表盘 | `daemons/monit_d` + `heapstore/` | 系统与业务指标、告警 |
| 日志终端 | 运行时日志服务 | 日志查看与终端模拟器 |
| 系统设置 | 本地配置 | 外观、网关、数据与版本信息 |
...
```
仓库:    https://atomgit.com/openairymax/desktop.git
分支:    main
